...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @role('Staff')
                        <x-nav-link :href="route('staff.portal')" :active="request()->routeIs('staff.portal')">
                            {{ __('Staff Portal') }}
                        </x-nav-link>
                    @endrole
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150 gap-2">
                            ...
                            @if(Auth::user()->avatar_path)
                                <img class="h-8 w-8 rounded-full object-cover border border-gray-200" src="{{ asset('storage/' . Auth::user()->avatar_path) }}" alt="{{ Auth::user()->name }}" />
                            @else
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @role('Staff')
                <x-responsive-nav-link :href="route('staff.portal')" :active="request()->routeIs('staff.portal')">
                    {{ __('Staff Portal') }}
                </x-responsive-nav-link>
            @endrole
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4 flex items-center gap-3">
                @if(Auth::user()->avatar_path)
                    <img class="h-10 w-10 rounded-full object-cover border border-gray-200" src="{{ asset('storage/' . Auth::user()->avatar_path) }}" alt="{{ Auth::user()->name }}" />
                @else
